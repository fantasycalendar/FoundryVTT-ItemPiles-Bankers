...
export function getCostOfVault(bankerActor) {
	...
	const canBuy = game.user.character ? game.itempiles.API.getPaymentData(vaultPrice, {
...
export async function createNewVault(bankerActor, vaultName) {
	...
	let folderStructure = false;
	if (flags?.folderName) {
		folderStructure = [];
		const folderParts = flags?.folderName.split("/");
		for(const part of folderParts){
			const result = Roll.replaceFormulaData(part.trim(), {
				banker: bankerActor.getRollData(),
				user: game.user.toObject()
			}, {
				warn: false
			});
			if(!result){
			...
			folderStructure.push(result);
		}
	}

	if (!folderStructure) {
		folderStructure = [bankerActor.name, `${game.user.name}`];
	}
